...
class EDSR(nn.Module):
    def __init__(self, num_channels, base_channel, upscale_factor, num_residuals):
        ...
        self.input_conv = nn.Conv2d(9, base_channel, kernel_size=3, stride=1, padding=1)
        ...
        for _ in range(int(math.log2(upscale_factor))):
            upscale.append(PixelShuffleBlock(base_channel, base_channel, upscale_factor))
        self.upscale_layers = nn.Sequential(*upscale)

        self.pre_output_conv = nn.Conv2d(base_channel, 32, kernel_size=3, stride=1, padding=1)
        self.output_conv = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)
